...
var (
	baseURL         = "wss://openws.bitmart.com"
...
func WsDepthServe(symbol string, handler WsDepthHandler, errHandler ErrHandler) (doneC, stopC chan struct{}, err error) {
	...
	wsHandler := func(message []byte) {
		...
		event.Subscribe = j.Get("subscribe").MustString()
		event.Symbol = j.Get("symbol").MustString()
		event.Precision = j.Get("precision").MustInt64()
                event.Data = make(map[string][]Tick)
		data := j.Get("data").MustMap()

                buys := data["buys"].([]interface{})
		bidsLen := len(buys)
		event.Data["buys"] = make([]Tick, bidsLen)
		for i := 0; i < bidsLen; i++ {
			item := buys[i].(map[string]interface{})
			event.Data["buys"][i] = Tick{
				Price:    item["price"].(string),
				Amount:   item["amount"].(string),
			}
		}
                sells := data["sells"].([]interface{})
		asksLen := len(sells)
		event.Data["sells"] = make([]Tick, asksLen)
		for i := 0; i < asksLen; i++ {
			item := sells[i].(map[string]interface{})
			event.Data["sells"][i] = Tick{
				Price:    item["price"].(string),
				Amount:   item["amount"].(string),
			}
		}
